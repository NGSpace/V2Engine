...
public class DefaultV2VariableParser implements IV2VariableParser {
	...
	@Override public AV2Value parse(V2Runtime runtime, String valuee, AV2Compiler comp, int line, int charpos) throws CompileException {
		...
		if (value.matches("\\[[\\s\\S]*\\]")) {
			...
			// Sends the text between the square brackets to Utils.processParemeters to tokenize the values.
			
			return new V2Array(Utils.processParemeters(value.substring(1, value.length()-1).replace("\n", "")),
		...
		if (value.matches("[A-Za-z\\d][A-Za-z\\d_]*")) return new V2DynamicVar(value, runtime, line, charpos);
		...
		for (int i=1;i<value.length(); i++) {
			...
			if (c=='.') {
				...
				for (int j=1;j<classyobjname.length(); j++) {
					...
					if (Character.isDigit(cc)||cc==' ') continue;
					if (cc=='*'||cc=='+'||cc=='-'||cc=='/'||cc=='%') {
						classyobjname = "";
						functionOrObject = "";
						break;
		...
		for (int i = 0;i<value.length();i++) {
			char c = value.charAt(i);
			if (mathvalue.length()==0&&c==' ') continue;
		...
		if (values.length>1) return new V2LogicalOR(values, line, charpos, value, comp);
		
		
		
		//Logical AND operator
		values = logicalOperator('&', value, runtime, line, charpos);
		if (values.length>1) return new V2LogicalAND(values, line, charpos, value, comp);
